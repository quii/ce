...
- **gremlins needs `--integration`, or it silently produces false-negative `LIVED` verdicts for exactly the same kind of code `--coverpkg` fixes above.** By default (`internal/engine/executor.go`), when checking whether a mutant survives, gremlins runs `go test` scoped to *only the package containing the mutated file*, not the whole module (`--integration` is what switches that target to `./...`). Since most of this project's domain/use-case packages have no `_test.go` files of their own - they're only exercised through `specifications` - that scoped test run trivially has nothing to fail, so the mutant is reported `LIVED` regardless of whether it's actually caught elsewhere. Caught for real while re-running the `greet-by-name` story through the migrated tool for the first time: gremlins reported `internal/domain/greeting.go`'s `Blank()` and a `specifications/greeting.go` assertion as `LIVED`; manually applying each mutation and running `go test ./...` directly failed 12/12 affected subtests across both drivers, proving both were real false negatives, not real gaps. Adding `--integration` resolved both to `KILLED` with no other change.
