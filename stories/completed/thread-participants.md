...
2. `participants` is computed once, when the `ConversationStarted` event is applied to build the projection, and stays fixed thereafter - posting a reply never changes it (the same "frozen participant set" guarantee reply-to-a-thread's rule 3 already relies on).
3. The write side is unaffected: `POST /conversations` still takes `recipients` as an input field, and the `ConversationStarted` event still records author and recipients separately in the durable event log - only the read-side representation changes shape. Nothing about starting a conversation or replying to a thread changes from a caller's point of view on the write path.
...
- Rule 1 (single field, the union): `domain.ThreadView.Participants` (`internal/domain/conversation_view.go`) replaces the old `Author`/`Recipients` fields. The union itself is computed by `ConversationStarted.Participants()` (`internal/domain/conversation.go`), the one place it's derived - both adapters call it rather than each re-deriving the union independently.
- Rule 2 (computed once, frozen): `applyConversationStarted` (`internal/adapters/memory/projection.go`, `internal/adapters/postgres/projection.go`) sets `Participants` only when a `ConversationStarted` event is applied; `applyReplyPosted` in both files never touches it.
- Rule 3 (write side unaffected): `StartConversationCommand`/`StartConversationParams`/the `ConversationStarted` event still carry `Recipients` exactly as before this story. The Postgres migration (`internal/adapters/postgres/migrations/00003_thread_participants.sql`) only changes the read-side `conversation_projection` table (replacing its `thread_author`/`recipients` columns with one `participants` column, data-preserving in both directions) - `conversation_events`/`conversation_outbox` are untouched.
