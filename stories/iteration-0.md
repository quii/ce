...
- [x] `revive`'s `argument-limit`, set to 2, scoped to `internal/ports/in`/`internal/ports/out` via a `linters.exclusions.rules` path/linters/text entry covering every other top-level package (RE2, which golangci-lint uses, has no negative lookahead, so "everywhere except X" has to be spelled out rather than expressed directly). **Corrected from the original plan**: the rule's own per-rule `exclude:` field (nested under `settings.revive.rules[]`) was tried first and looked structurally correct, but does nothing in golangci-lint v2.12.2 - verified empirically by adding a 4-arg function to a top-level file, a nested-subdirectory file, and an adapter file, all three still flagged despite being listed there. Only the positive case (`ports/in` correctly flagged) had actually been checked before; the "elsewhere excluded" half of the original claim was never verified and was false. Switched to the same `linters.exclusions.rules` mechanism `file-length-limit` already uses successfully below, and reverified all four cases (three excluded paths silent, `ports/in` still flagged) before trusting it
