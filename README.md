...
`mage` and every tool it shells out to (`golangci-lint`, `go-mutesting`) are tracked via `go.mod`'s `tool` directive, not installed globally - run everything through `go tool mage <target>` so it works regardless of `PATH`/`GOBIN` setup:
